...
function polyPuffRound(ctx, cx, cy, r, cLight, cDark) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + Math.cos(a0) * r, cy + Math.sin(a0) * r * 0.86);
    ctx.lineTo(cx + Math.cos(a1) * r, cy + Math.sin(a1) * r * 0.86);
...
function drawCottonCandy(ctx, x, y, s, twist) {
  ...
  ctx.strokeStyle = '#E8C9A0'; ctx.lineWidth = 3 * s; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0, 16 * s); ctx.lineTo(0, 16 * s + stickH); ctx.stroke();
  COTTON_PUFFS.forEach(p => {
    const pal = COTTON_PALETTES[p.c];
    polyPuffRound(ctx, p.dx * s, p.dy * s, p.r * s, pal.light, pal.dark);
  });
  ctx.restore();
}
...
export default {
  ...
  drawDecor(ctx, api) {
    ...
    L.crystals.forEach(c => {
      ...
      ctx.beginPath(); ctx.moveTo(cx2 - 2, top + 22); ctx.lineTo(cx2 + 4, top + 34); ctx.lineTo(cx2 - 2, top + 46); ctx.lineTo(cx2 - 8, top + 34); ctx.closePath(); ctx.fill();
    });
    ...
    L.hills.forEach(h => {
      ...
      ctx.fillStyle = left;
      ctx.beginPath();
      ctx.moveTo(hx - h.w / 2, base); ctx.lineTo(hx - h.w * 0.06, domeTop); ctx.lineTo(hx, base - h.h * 0.88); ctx.lineTo(hx, base);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = right;
      ctx.beginPath();
      ctx.moveTo(hx, base); ctx.lineTo(hx, base - h.h * 0.88); ctx.lineTo(hx + h.w * 0.1, domeTop); ctx.lineTo(hx + h.w / 2, base);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,.4)';
      ctx.beginPath(); ctx.ellipse(hx - h.w * 0.12, domeTop + h.h * 0.22, h.w * 0.1, h.h * 0.14, -0.4, 0, Math.PI * 2); ctx.fill();
    });
